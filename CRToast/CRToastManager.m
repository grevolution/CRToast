...
+ (void)showNotificationWithOptions:(NSDictionary *)options completionBlock:(void (^)(void))completion {
    [[CRToastManager manager]
        addNotification:[CRToast notificationWithOptions:options appearanceBlock:nil completionBlock:completion]];
}

+ (void)showNotificationWithMessage:(NSString *)message completionBlock:(void (^)(void))completion {
    [self showNotificationWithOptions:@{ kCRToastTextKey : message } completionBlock:completion];
}
...
CRToastAnimationCompletionBlock CRToastOutwardAnimationsCompletionBlock(CRToastManager *weakSelf) {
    return ^void(BOOL completed) {
      ...
      if (weakSelf.notification.completion)
          weakSelf.notification.completion();
      ...
      switch (weakSelf.notification.outAnimationType) {
          ...
          case CRToastAnimationTypeGravity: {
              ...
              UIGravityBehavior *gravity =
                  [[UIGravityBehavior alloc] initWithItems:@[ weakSelf.notificationView, weakSelf.statusBarView ]];
              ...
              if (notification.presentationType == CRToastPresentationTypePush)
                  [collisionItems addObject:weakSelf.statusBarView];
...
- (void)displayNotification:(CRToast *)notification {
    ...
    // changing the y of the notification frame based on prefferedTopMargin
    containerFrame.origin.y += notification.preferredTopMargin;
    ...
    [self showNotification:notification
                  inwardAnimationBlock:inwardAnimationsBlock
        inwardCompletionAnimationBlock:inwardAnimationsCompletionBlock];

    if (notification.text.length > 0 || notification.subtitleText.length > 0) {
        // Synchronous notifications (say, tapping a button that presents a toast) cause VoiceOver to read the button
        // immediately, which interupts the toast. A short delay (not the best solution :/) allows the toast to interupt
        // the button.
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
          UIAccessibilityPostNotification(UIAccessibilityAnnouncementNotification,
                                          [NSString stringWithFormat:@"Alert: %@, %@", notification.text ?: @"", notification.subtitleText ?: @""]);
        });
    }
}
...
- (void)showNotification:(CRToast *)notification
    ...
    switch (notification.inAnimationType) {
        ...
        case CRToastAnimationTypeGravity: {
            ...
            if (notification.presentationType == CRToastPresentationTypePush)
                [collisionItems addObject:statusBarView];
            UICollisionBehavior *collision = [[UICollisionBehavior alloc] initWithItems:collisionItems];
